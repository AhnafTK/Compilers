 ...
 * -------------------------------------------------------------
 */
zeus_void bErrorPrint(zeus_char* fmt, ...);
zeus_void displayBuffer(BufferReader* ptr_Buffer);
zeus_long getFileSize(zeus_char* fname);
zeus_intg isNumber(const zeus_char* ns);
zeus_void startReader(zeus_char*, zeus_char*, zeus_char, zeus_intg, zeus_intg);
...
zeus_intg mainReader(zeus_intg argc, zeus_char** argv) {
	...
	zeus_char* program = argv[0];
	zeus_char* input = argv[2];
	zeus_char mode = MODE_MULTI;
	zeus_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
zeus_void startReader(zeus_char* program, zeus_char* input, zeus_char mode, zeus_intg size, zeus_intg increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	zeus_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	zeus_char symbol;			/* symbol read from input file */
