...
zeus_intg errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern zeus_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern zeus_intg line; /* source code line number - defined in scanner.c */
...
extern zeus_void startParser(zeus_void);
extern zeus_intg startScanner(ReaderPointer sc_buf);

static zeus_void printParserError(zeus_char* fmt, ...);
static zeus_void displayParser(ReaderPointer ptrBuffer);
static zeus_long getParserFileSize(zeus_char* fname);
static zeus_void callGarbageCollector(zeus_void);
...
zeus_intg mainParser(zeus_intg argc, zeus_char** argv) {
	...
	zeus_intg loadsize = 0; /*the size of the file loaded in the buffer */
...
zeus_void printParserError(zeus_char* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(zeus_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
zeus_long getParserFileSize(zeus_char* fname) {
	FILE* input;
	zeus_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
zeus_void displayParser(ReaderPointer ptrBuffer) {
...
zeus_void callGarbageCollector(zeus_void) {
